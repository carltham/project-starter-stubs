...
export interface UserDAO {
  ...
  retrieveAll(searchParams: { title?: string; published?: boolean }): Promise<UserDTO[]>;
...
export class UserDAOImpl implements UserDAO {
  ...
  retrieveAll(searchParams: { title?: string; published?: boolean }): Promise<UserDTO[]> {
    ...
    return new Promise<UserDTO[]>((resolve, reject) => {
      let users: UserDTO[];
      this.connection.query<UserEntity[]>(query, (err: any, res: UserEntity[]) => {
  ...
  deleteAll(): Promise<number> {
    return new Promise((resolve, reject) => {
      this.connection.query<ResultSetHeader>('DELETE FROM users', (err: any, res: any) => {
        if (err) reject(err);
        else resolve(res.affectedRows);
      });
    });
